...
#include "config.h"
  ...
  #define CASPTR16(A, B, C)     __sync_bool_compare_and_swap_16((__int128*) A,*((__int128*)&B),*((__int128*)&C))
  ...
  enum {NO_PARALLELISM_IN_SUBTREE, 
        ...
        LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG};
  ...
  ///// BENCHMARKING /////
        ...
        // --------------------------------------
         ...
         struct timeval receive_buffer_helping;
         struct timeval help_fill_receive_buffer;
         struct timeval fill_rec_buff_tree_construction;
         struct timeval phase1_time_per_thread[80];
        // --------------------------------------
        ...
        double total_time;
        double total_help_fill_rec_buf_time;
        double total_phase1_2;
        double phase1_time_per_thread2[80];
        ...
        #define INIT_STATS() total_input_time = 0;\
                             ...
                             total_help_fill_rec_buf_time=0;\
                             num_of_helped_series=0;\
                             total_phase1_2=0;\
                             pq_size = 0;\
                             for(int i =0;i<80;i++) phase1_time_per_thread2[i] = 0;
        // printf("input\t output\t nodes\t checked_nodes\t bytes_accessed\t loaded_nodes\t loaded_records\t approximate_distance\t distance\t total\n");
        #define PRINT_STATS(result_distance) printf("%lf\t %lf\t %d\t %d\t %ld\t %d\t %lld\t %lf\t %lf\t %lf\t %d\t %lf\t %lf\t %ld\n", \
        ...
        block_help_avoided, fill_rec_bufs_time, create_tree_index_time,num_of_helped_series);
        ...
        #define MY_PRINT_STATS(result_distance) printf("%lf\t %lf\t %lf\t %lf\t %lf\t %lf\t  %lu\t %lu\t %lu\t %lu\t %ld\t %ld\t %ld\t %ld\t %ld\t %ld\t %lf\t %lf\t %lf\t %lf\t %lf\t %lf\t %lf\t %lf\t %ld\t %lf\t %ld\t\n", \
        total_input_time, total_output_time, \
        ...
        queue_fill_time + queue_process_time + initialize_index_tree_time,total_help_fill_rec_buf_time,num_of_helped_series,fill_rec_bufs_time+create_tree_index_time,pq_size);

        #define PRINT_STATS_TO_FILE(result_distance, fp) fprintf(fp, "%lf\t %lf\t %lf\t %lf\t %lf\t %lf\t %lu\t %lu\t %lu\t %lu\t %ld\t %ld\t %ld\t %ld\t %ld\t %ld\t %lf\t %lf\t %lf\t %lf\t %lf\t %lf\t %lf\t %lf\t %ld\t %lf\t %ld\t\n", \
        total_input_time, total_output_time, \
        total_time, fill_rec_bufs_time, create_tree_index_time, query_answering_time, \
        blocks_helped, block_help_avoided, subtrees_helped, subtree_help_avoided, \
        ts_in_RecBufs_cnt, ts_in_RecBufs_cnt-dataset_size, ts_in_tree_cnt-ts_in_RecBufs_cnt, \
        ...
        #define COUNT_HELP_REC_BUFFER gettimeofday(&help_fill_receive_buffer, NULL);
        #define COUNT_PHASE1_TIME_PER_THREAD(x) gettimeofday(&phase1_time_per_thread[x],NULL);

        #define COUNT_PHASE1_TIME_END_PER_THREAD(x)  gettimeofday(&current_time, NULL); \
                                                  tS = phase1_time_per_thread[x].tv_sec*1000000 + (phase1_time_per_thread[x].tv_usec); \
                                                  tE = current_time.tv_sec*1000000 + (current_time.tv_usec); \
                                                  phase1_time_per_thread2[x] += (tE - tS); 
        ...
        #define COUNT_HELP_FILL_REC_BUF_TIME_END  gettimeofday(&current_time, NULL);  \
                                      tS = help_fill_receive_buffer.tv_sec*1000000 + (help_fill_receive_buffer.tv_usec); \
                                      tE = current_time.tv_sec*1000000  + (current_time.tv_usec); \
                                      total_help_fill_rec_buf_time += (tE - tS);                                
