...
typedef struct parallel_fbl_soft_buffer_ekosmas_lf_geopat {
    isax_node_single_buffer ** volatile node;                       // EKOSMAS: ADDED 'volatile' JUNE 16 2020 - CHANGED JUNE 24 2020 //GEOPAT Storing roots of subtrees
    sax_type **sax_records;
    file_position_type **pos_records;
    root_mask_type *masks;
    sax_type **sax_cardinalities;
    volatile unsigned char initialized;             // EKOSMAS: ADDED 'volatile'
    int *max_buffer_size;                           // EKOSMAS: Why is this a pointer?
    int *buffer_size;                               // EKOSMAS: Why is this a pointer?
    volatile unsigned char processed;               // EKOSMAS: CHANGED from finished to processed
    volatile unsigned long *iSAX_processed;   
    int *finish_receive_buffer;

    Update **rootSubtreesUpdateArray;

} parallel_fbl_soft_buffer_ekosmas_lf_geopat;
...
typedef struct parallel_first_buffer_layer_ekosmas_lf_geopat {
    int number_of_buffers;
    int initial_buffer_size;
    int max_total_size;
    int current_record_index;
    parallel_fbl_soft_buffer_ekosmas_lf_geopat *soft_buffers;
} parallel_first_buffer_layer_ekosmas_lf_geopat;
...
parallel_first_buffer_layer_ekosmas_lf_geopat * initialize_pRecBuf_ekosmas_lf_geopat(int initial_buffer_size, int max_fbl_size, 
                                    int max_total_size, isax_index *index);
