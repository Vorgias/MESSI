                            ...
                            query_result (*search_function)(ts_type*, ts_type*, isax_index*,node_list*, float, const char, const int, const int query_id)) 
{
    ...
    node_list nodelist;
    nodelist.nlist = malloc(sizeof(void *) * pow(2, index->settings->paa_segments));
    nodelist.node_amount = 0;
...
//geopat
void isax_query_binary_file_traditional_ekosmas_lf_geopat(const char *ifilename,const char *output_file, int q_num, isax_index *index,
                            float minimum_distance, const char parallelism_in_subtree, const int third_phase,
                            query_result (*search_function)(ts_type*, ts_type*, isax_index*,node_list*, float, const char, const int, const int query_id)) 
{
    COUNT_INPUT_TIME_START
    ...
    int num_of_subtrees = pow(2, index->settings->paa_segments);
    node_list  nodelist;
    nodelist.nlist = malloc(sizeof(void*) * num_of_subtrees);
    nodelist.node_amount = 0 ;
    ...
    for(int i = 0 ;i < num_of_subtrees;i++){
        nodelist.nlist[i] = current_fbl_node->node[i];
    }
    ...
    for(int k = 0 ; k < number_of_subtrees; k++){  //reset fai counters
        if(current_fbl_node->node[k] != NULL){
            int nodes = current_fbl_node->node[k]->subTreeNodesNum + 1;
            current_fbl_node->node[k]->rootProcessedNodes = malloc(sizeof(int) *nodes);
            for(int j = 0 ; j < nodes ; j++){
                current_fbl_node->node[k]->rootProcessedNodes[j] = 0;
            }
        }
     }
    ...
    while (q_loaded < q_num)
    {
        ...
        COUNT_OUTPUT_TIME_END
        PRINT_QUERY_STATS(result.distance);
        //validate_bsf(output_file,result.distance,q_loaded-1);
...
void isax_query_binary_file_traditional_ekosmas_lf_geopat_validation(const char *ifilename,const char *output_file, int q_num, isax_index *index,
                            float minimum_distance, const char parallelism_in_subtree, const int third_phase,unsigned long timeSeriesNum,
                            query_result (*search_function)(ts_type*, ts_type*, isax_index*,node_list*, float, const char, const int, const int query_id,unsigned long timeSeriesNum)) 
{
    COUNT_INPUT_TIME_START
    fprintf(stderr, ">>> Performing queries in file: %s\n", ifilename);
    FILE * ifile;
    ...
    int num_of_subtrees = pow(2, index->settings->paa_segments);
    node_list  nodelist;
    /*nodelist.nlist = malloc(sizeof(isax_node *) * num_of_subtrees);
    nodelist.node_amount = 0 ;

    parallel_first_buffer_layer_ekosmas_lf_geopat *fbl = (parallel_first_buffer_layer_ekosmas_lf_geopat *)(index->fbl);
    parallel_fbl_soft_buffer_ekosmas_lf_geopat *current_fbl_node = (fbl->soft_buffers);
    nodelist.nlist = current_fbl_node->node;
*/
    ...
    while (q_loaded < q_num)
    {
        ...
        paa_from_ts(ts, paa, index->settings->paa_segments,
                    index->settings->ts_values_per_paa_segment);
        query_result result = search_function(ts, paa, index, &nodelist, minimum_distance, parallelism_in_subtree, third_phase, q_loaded,timeSeriesNum);
        q_loaded++;
