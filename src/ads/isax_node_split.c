...
void split_node_inmemory(isax_index *index, isax_node *node) 
{
    ...
    if (node->parent == NULL) {
        ...
        for (i=0; i<index->settings->paa_segments; i++) {
            split_data->split_mask[i] = ((isax_node*)node->parent)->split_data->split_mask[i];
        }
    }
                                                 ...
                                                 const unsigned char local_flag)
{
    ...
    else if ((isax_node*)parent->left_child == node) {
        node_to_change = ((isax_node * volatile*)&parent->left_child);
    }
    else if ((isax_node*)parent->right_child == node) {
        node_to_change = ((isax_node * volatile*)&parent->right_child);
    }
    ...
    // have not yet acqueired a potision into node->buffer.
    isax_node_record *split_buffer = malloc(sizeof(isax_node_record) * 
                                            (index->settings->max_leaf_size + total_workers_num));
                                                 ...
                                                 unsigned long my_id)
{
    ...
    if (!parent) {
        // if (!current_fbl_node) {
        //     printf("Thread [%d]: ERROR! current_fbl_node can not be NULL!!!!\n");fflush(stdout);
        //     getchar();
        // }
        node_to_change = &current_fbl_node->node;
        // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - CAS - decide address - root node\n", my_id);fflush(stdout);
    }
    else if (parent->left_child == node) {
        node_to_change = ((isax_node * volatile*)&parent->left_child);
        // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - CAS - decide address - left child\n", my_id);fflush(stdout);
    }
    else if (parent->right_child == node) {
         node_to_change = ((isax_node * volatile*)&parent->right_child);
        // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - CAS - decide address - right child\n", my_id);fflush(stdout);
    }
    else { // node has been already replaced by some other thread
        // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - CAS - decide address - NULL address!!!!!!\n", my_id);fflush(stdout);
        return parent;          // EKOSMAS AUGUST 01, 2020: Is this correct?
    }
    ...
    // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - created new_node\n", my_id);fflush(stdout);
    ...
    // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - 2\n", my_id);fflush(stdout);
    ...
    // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - 3\n", my_id);fflush(stdout);

    
    // index->memory_info.mem_tree_structure += 2;              // EKOSMAS 28 JULY, 2020: removed
    
    isax_node * left_child = isax_leaf_node_init_lockfree_cow_split(index->settings->initial_leaf_buffer_size);
    // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - created left child\n", my_id);fflush(stdout);
    
    isax_node * right_child = isax_leaf_node_init_lockfree_cow_split(index->settings->initial_leaf_buffer_size);
    // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - created right child\n", my_id);fflush(stdout);
    ...
    // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - buffers copy - END\n", my_id);fflush(stdout);
    ...
    // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - split point decided\n", my_id);fflush(stdout);
    ...
    // printf ("Thread [%d]: added [%d] to left child and [%d] to right child\n", my_id, left_child->fai_leaf_size, right_child->fai_leaf_size);
    // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - children's buffers created\n", my_id);fflush(stdout);

    free(split_buffer);

    // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - CAS - apply CAS\n", my_id);fflush(stdout);
    ...
    // if (node_to_change) {
    //     if (*node_to_change == node && CASPTR(node_to_change, node, new_node)) {
    //         // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - CAS - apply CAS - success\n", my_id);fflush(stdout);
    //         return new_node;
    //     }
    //     else {
    //         // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - CAS - apply CAS - failed\n", my_id);fflush(stdout);
...
isax_node_single_buffer *split_node_inmemory_parallel_lockfree_cow_geopat(isax_index *index, isax_node_single_buffer *node, 
                                                 parallel_fbl_soft_buffer_ekosmas_lf_geopat *current_fbl_node,
                                                 unsigned long my_id,root_mask_type my_mask,int function_type)
{

    // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - CAS - decide address\n", my_id);fflush(stdout);

    isax_node_single_buffer * volatile *node_to_change = NULL;
    isax_node_single_buffer *parent = (isax_node_single_buffer*) node->node->parent;
    // CAS on parent....
    if (!parent) {
        // if (!current_fbl_node) {
        //     printf("Thread [%d]: ERROR! current_fbl_node can not be NULL!!!!\n");fflush(stdout);
        //     getchar();
        // }
        node_to_change = &current_fbl_node->node[(int)my_mask];
        // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - CAS - decide address - root node\n", my_id);fflush(stdout);
    }
    else if (((isax_node_single_buffer*)parent->node->left_child) == node) {
        node_to_change = ((isax_node_single_buffer * volatile*)&parent->node->left_child);
        // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - CAS - decide address - left child\n", my_id);fflush(stdout);
    }
    else if (((isax_node_single_buffer*)parent->node->right_child) == node) {
        node_to_change = ((isax_node_single_buffer * volatile*)&parent->node->right_child);
    ...
    isax_node_single_buffer *new_node; 
    if (!parent) {
        new_node = isax_root_node_init_lockfree_cow_copy_geopat(node, node->node->mask, index->settings->initial_leaf_buffer_size,index->settings->max_leaf_size,function_type);
    }
    else {
        new_node = isax_leaf_node_init_lockfree_cow_copy_geopat(node, index->settings->initial_leaf_buffer_size,index->settings->max_leaf_size,function_type);
    }

    new_node->node->parent = (isax_node*)node->node->parent;
    ...
    if (node->node->parent == NULL) {
        ...
        for (i=0; i<index->settings->paa_segments; i++) {
            split_data->split_mask[i] = ((isax_node_single_buffer*)node->node->parent)->node->split_data->split_mask[i];
        }
    }
    ...
    isax_node_single_buffer * left_child = isax_leaf_node_init_lockfree_cow_split_geopat(index->settings->initial_leaf_buffer_size,index->settings->max_leaf_size,function_type);
    // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - created left child\n", my_id);fflush(stdout);
    
    isax_node_single_buffer * right_child = isax_leaf_node_init_lockfree_cow_split_geopat(index->settings->initial_leaf_buffer_size,index->settings->max_leaf_size,function_type);
    // printf("Thread [%d]: Populating SubTree (Lock Free) - START - adding iSAX - START - split - START - created right child\n", my_id);fflush(stdout);
    
    left_child->node->parent = new_node;                              
    right_child->node->parent = new_node;                            

    new_node->node->split_data = split_data;
    new_node->node->left_child = left_child;
    new_node->node->right_child = right_child;
    ...
    // for (i=node->buffer->partial_buffer_size-1; i>=0; i--) {
    for (i=buffer_size-1; i>=0; i--) {
        split_buffer[split_buffer_index].sax = node->node->buffer->partial_sax_buffer[i];
        split_buffer[split_buffer_index].ts = NULL;
        split_buffer[split_buffer_index].position = node->node->buffer->partial_position_buffer[i];
        split_buffer[split_buffer_index].insertion_mode = NO_TMP | PARTIAL;                         // EKOSMAS JULY 30, 2020: is this required???
        split_buffer_index++;
    }
    ...
    while (split_buffer_index > 0) {
        split_buffer_index--;
        if(mask & split_buffer[split_buffer_index].sax[split_data->splitpoint]) {   
            add_record_to_node_inmemory_parallel_lockfree_cow_local_geopat(index, right_child, &split_buffer[split_buffer_index], my_id,function_type);
        }
        else {
            add_record_to_node_inmemory_parallel_lockfree_cow_local_geopat(index, left_child, &split_buffer[split_buffer_index], my_id,function_type);
        }
    }
    ...
    isax_tree_destroy_lockfree_geopat(new_node);   //  it also destroys left and right child!
    return *node_to_change;
}
