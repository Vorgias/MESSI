...
isax_node * insert_to_single_pRecBuf_geopat(parallel_first_buffer_layer_ekosmas_lf_geopat *fbl, sax_type *sax,
                          file_position_type *pos,root_mask_type mask,
                          isax_index *index, pthread_mutex_t *lock_firstnode, int workernumber,int total_workernumber,unsigned long number_of_series,unsigned long time_series_num,int flag)
{
    parallel_fbl_soft_buffer_ekosmas_lf_geopat *current_buffer = (parallel_fbl_soft_buffer_ekosmas_lf_geopat*)fbl->soft_buffers;
    
    
    memcpy((void*)current_buffer->sax_records[time_series_num],(void*)sax,index->settings->sax_byte_size);
    memcpy((void*)current_buffer->pos_records[time_series_num],(void*)pos,index->settings->position_byte_size);

 
    current_buffer->masks[time_series_num] = mask;
    if(flag == 0){
        free(sax);
        free(pos);
    }
