...
int main (int argc, char **argv)
{
    ...
    if (function_type == 0) {   // Botao's version of MESSI
        ...
        if (function_type == 9990){                                     // ekosmas's version according to botao's
        ...
        else if (function_type == 9992 || function_type == 99929){      // ekosmas's lock-free full fai version, with tree copy (i.e. no parallelism) on subtrees
            DO_NOT_HELP = 0;
            ...
            // PHASE 3: Query Answering
            isax_query_binary_file_traditional_ekosmas_lf(queries, queries_size, idx, minimum_distance, NO_PARALLELISM_IN_SUBTREE, 1, &exact_search_ParISnew_inmemory_hybrid_ekosmas_lf);
        }
        else if (function_type == 9993 || function_type == 99939){      // ekosmas's lock-free fai only after help version, with tree copy (i.e. no parallelism) on subtrees
            DO_NOT_HELP = 0;
            ...
            // PHASE 3: Query Answering
            isax_query_binary_file_traditional_ekosmas_lf(queries, queries_size, idx, minimum_distance, NO_PARALLELISM_IN_SUBTREE, 0, &exact_search_ParISnew_inmemory_hybrid_ekosmas_lf);
        }
        else if (function_type == 9994 || function_type == 99949){      // ekosmas's lock-free full fai version with blocking parallelism in subtree
            DO_NOT_HELP = 0;
            ...
            // PHASE 3: Query Answering
             isax_query_binary_file_traditional_ekosmas_lf(queries, queries_size, idx, minimum_distance, BLOCKING_PARALLELISM_IN_SUBTREE, 1, &exact_search_ParISnew_inmemory_hybrid_ekosmas_lf);
        }
        else if (function_type == 9995 || function_type == 99959){      // ekosmas's lock-free fai only after help version with blocking parallelism in subtree
            DO_NOT_HELP = 0;
            ...
            // PHASE 3: Query Answering
            isax_query_binary_file_traditional_ekosmas_lf(queries, queries_size, idx, minimum_distance, BLOCKING_PARALLELISM_IN_SUBTREE, 0, &exact_search_ParISnew_inmemory_hybrid_ekosmas_lf);
        }
        else if (function_type == 9996 || function_type == 99969){      // ekosmas's lock-free full fai version with lock-free parallelism in subtree and announce
            DO_NOT_HELP = 0;
            ...
            // PHASE 3: Query Answering
            isax_query_binary_file_traditional_ekosmas_lf(queries, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_ANNOUNCE, 1, &exact_search_ParISnew_inmemory_hybrid_ekosmas_lf);
        }
        else if (function_type == 9997 || function_type == 99979){      // ekosmas's lock-free fai only after help version with lock-free parallelism in subtree and announce
            DO_NOT_HELP = 0;
        ...
        else if (function_type == 99966 || function_type == 999669){      // ekosmas's lock-free full fai version with lock-free parallelism in subtree and announce, only after helping and per subtree
            DO_NOT_HELP = 0;
            ...
            // PHASE 3: Query Answering
            isax_query_binary_file_traditional_ekosmas_lf(queries, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_ANNOUNCE_AFTER_HELP, 1, &exact_search_ParISnew_inmemory_hybrid_ekosmas_lf);
        }
        else if (function_type == 99977 || function_type == 999779){      // ekosmas's lock-free fai only after help version with lock-free parallelism in subtree and announce, only after helping and per subtree
            DO_NOT_HELP = 0;
            ...
            isax_query_binary_file_traditional_ekosmas_lf(queries, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_ANNOUNCE_AFTER_HELP, 1, &exact_search_ParISnew_inmemory_hybrid_ekosmas_lf);
                 ...
                 function_type == 99966697 || function_type == 99966698){      // ekosmas's lock-free full fai version with lock-free parallelism in subtree and announce, only after helping and per leaf
            DO_NOT_HELP = 0;
            if ((function_type%100)/10 == 9) {
                DO_NOT_HELP = 1;
            }
            // PHASE 1: Fill in of Receive Bufers and PHASE 2: Index Creation
            // printf ("PHASE 1: Fill in Rec Bufs!!\n"); fflush(stdout);
            ...
            // PHASE 3: Query Answering
            // printf ("PHASE 3: Query Answring!!\n"); fflush(stdout);
            isax_query_binary_file_traditional_ekosmas_lf(queries, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_ANNOUNCE_AFTER_HELP_PER_LEAF, function_type%10, &exact_search_ParISnew_inmemory_hybrid_ekosmas_lf);
        }
        else if (function_type == 999666010 || function_type == 999666011 || function_type == 999666012 || function_type == 999666013 ||
                 function_type == 999666910 || function_type == 999666911 || function_type == 999666912 || function_type == 999666913) {      // ekosmas's lock-free full fai version with lock-free parallelism in subtree and announce, only after helping and per leaf
            DO_NOT_HELP = 0;
            ...
            // PHASE 3: Query Answering
            // printf ("PHASE 3: Query Answring!!\n"); fflush(stdout);
            isax_query_binary_file_traditional_ekosmas_lf(queries, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_ANNOUNCE_AFTER_HELP_PER_LEAF, function_type%100, &exact_search_ParISnew_inmemory_hybrid_ekosmas_lf);
        }
                 ...
                 function_type == 99977797 || function_type == 99977798){      // ekosmas's lock-free full fai version with lock-free parallelism in subtree and announce, only after helping and per leaf
            DO_NOT_HELP = 0;
            if ((function_type%100)/10 == 9) {
                DO_NOT_HELP = 1;
            }
            ...
            isax_query_binary_file_traditional_ekosmas_lf(queries, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_ANNOUNCE_AFTER_HELP_PER_LEAF, function_type%10, &exact_search_ParISnew_inmemory_hybrid_ekosmas_lf);
                 ...
                 function_type == 999777910 || function_type == 999777911 || function_type == 999777912 || function_type == 999777913) {      // ekosmas's lock-free full fai version with lock-free parallelism in subtree and announce, only after helping and per leaf
            DO_NOT_HELP = 0;
            if ((function_type%1000)/100 == 9) {
                DO_NOT_HELP = 1;
            } 
        ...
        // else if (function_type == 999777 || function_type == 9997779){      // ekosmas's lock-free fai only after help version with lock-free parallelism in subtree and announce, only after helping and per leaf
        //     DO_NOT_HELP = 0;
        //     if (function_type == 9997779) {
        //         DO_NOT_HELP = 1;
        //     }            
        //     // PHASE 1: Fill in of Receive Bufers and PHASE 2: Index Creation
        //     index_creation_pRecBuf_new_ekosmas_lock_free_fai_only_after_help_with_lockfree_parallelism_in_subtree_announce_after_help_per_leaf(dataset, dataset_size, idx);
        ...
        //     // PHASE 3: Query Answering
        //     isax_query_binary_file_traditional_ekosmas_lf(queries, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_ANNOUNCE_AFTER_HELP_PER_LEAF, 0, &exact_search_ParISnew_inmemory_hybrid_ekosmas_lf);
        // }
        else if (function_type == 9998 || function_type == 99989){      // ekosmas's lock-free full fai version with lock-free parallelism in subtree and cow
            DO_NOT_HELP = 0;
            ...
            // PHASE 3: Query Answering
           isax_query_binary_file_traditional_ekosmas_lf(queries, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_COW, 10, &exact_search_ParISnew_inmemory_hybrid_ekosmas_lf);
        }
        else if(function_type == 99981) { //cyclic OR DO ALL OPT
             printf(" Long = %d int = %d\n",sizeof(long),sizeof(int));
            index_creation_pRecBuf_geopat_cyclic(dataset,dataset_size,idx,6,1);
            
            printf("Phase3\n");
            isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat);
        }
        else if(function_type == 99981000) { //FULL CAS
            printf(" Long = %d int = %d\n",sizeof(long),sizeof(int));
            index_creation_pRecBuf_geopat_FULL_CAS(dataset,dataset_size,idx,7,1);
              
            printf("Phase3\n");
            isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat);
        }
        else if(function_type == 9998100) { // DO ALL
             printf(" Long = %d int = %d\n",sizeof(long),sizeof(int));
            index_creation_pRecBuf_geopat_doall(dataset,dataset_size,idx,1,1);
            
            printf("Phase3\n");
            isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat);
        }
        else if(function_type == 99980){    //RawBuffer Results to file
            index_initialization_rawBuffer_geopat(dataset,dataset_size,idx, 0);
            
            isax_query_binary_file_traditional_ekosmas_lf_geopat_validation(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0,dataset_size, &find_distances);
        }//BENCHMARK_FULL_FAI_BACKOFF
        else if(function_type == 9998264){  //FULL FAI back off phase1
            index_creation_pRecBuf_geopat_fetch_and_add_phase1_backoff(dataset,dataset_size,idx,0,fai_step,backoff_multiplier);
            
            printf("Phase3\n");
           // isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat);
        }
        else if(function_type == 999800){ //Micro Benchmark
            printf("Micro Benchmark\n");
            index_creation_pRecBuf_geopat_fetch_and_add_phase1_backoff_microbenchmark(dataset,dataset_size,idx,0,1);
                   
            printf("Phase3\n");
           // isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat);
        }                    
        else if (function_type == 99982){  //full fetch and add phase1+phase2 step 1
            printf(" Long = %d int = %d\n",sizeof(long),sizeof(int));
            index_creation_pRecBuf_geopat_fetch_and_add_phase1(dataset,dataset_size,idx,0,fai_step);
               
           // printf("Phase3\n");
            isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat);
        }//---------------------------------Enhanched Versions EP of phase1 + phase2-------------------------------------------                                                                     
        else if ( function_type == 99983){  //  Emp parallel phase1 starting from start for helping. 
            printf(" Long = %d int = %d\n",sizeof(long),sizeof(int));
            index_creation_pRecBuf_geopat_ep(dataset,dataset_size,idx, 1);

            printf("Phase3\n");
            isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat);
        }
        else if (function_type == 99984){ //Emp parallel - Skipping chunks + Fetch and Add phase2
            printf(" Long = %d int = %d\n",sizeof(long),sizeof(int));
            index_creation_pRecBuf_geopat_ep_skip_chunks(dataset,dataset_size,idx,2);

            printf("Phase3\n");
            isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat);
        }
        else if (function_type == 99985){ //Skipping chunks and starting from where last started / Same for phase 2
            printf(" Long = %d int = %d , unsigned long =  %d\n",sizeof(long),sizeof(int),sizeof(unsigned long));
            index_creation_pRecBuf_geopat_ep_skip_chunks_opt(dataset,dataset_size,idx, 3);

            printf("Phase3\n");
            isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat);
        }//----------------------------Chunk Versions of best enhanched algorithm 99985--------------------------------------
        else if (function_type == 999855){ //Skipping chunks and starting from where last started / Same for phase 2
            printf(" Long = %d int = %d , unsigned long =  %d\n",sizeof(long),sizeof(int),sizeof(unsigned long));
            index_creation_pRecBuf_geopat_ep_skip_chunks_opt(dataset,dataset_size,idx, 3);

            printf("Phase3\n");
            isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat_multiple_queues);
        }
        else if (function_type == 99986){ 
            printf(" Long = %d int = %d , unsigned long =  %d\n",sizeof(long),sizeof(int),sizeof(unsigned long));
            index_creation_pRecBuf_geopat_chunk_size(dataset,dataset_size,idx, 4,chunk_size);

            printf("Phase3\n");
            isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat);
        }//----------------------------No Receive Buffer algorithms -----------------------------------------------
         else if (function_type == 9998200){ //Baseline Algorithm
            printf(" Long = %d int = %d , unsigned long =  %d\n",sizeof(long),sizeof(int),sizeof(unsigned long));
            index_creation_No_pRecBuf_geopat_1chunk_simple_help(dataset,dataset_size,idx, 1);

            printf("Phase3\n");
            isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat);
        }
        else if (function_type == 99982000){ //Baseline Algorithm
            printf(" Long = %d int = %d , unsigned long =  %d\n",sizeof(long),sizeof(int),sizeof(unsigned long));
            index_creation_No_pRecBuf_geopat_do_all_opt(dataset,dataset_size,idx, 1);

            printf("Phase3\n");
            isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat);
        }           
        else if (function_type == 999820){ //Best enhanchement
            printf(" Long = %d int = %d , unsigned long =  %d\n",sizeof(long),sizeof(int),sizeof(unsigned long));
            index_creation_No_pRecBuf_geopat(dataset,dataset_size,idx, 1);

            printf("Phase3\n");
            isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat);
        }
        else if (function_type == 999821){ //Chunk Algorithms
            printf(" Long = %d int = %d , unsigned long =  %d\n",sizeof(long),sizeof(int),sizeof(unsigned long));
            index_creation_No_pRecBuf_geopat_chunk_size(dataset,dataset_size,idx,1,chunk_size);


            printf("Phase3\n");
            isax_query_binary_file_traditional_ekosmas_lf_geopat(queries,dataset_output, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_FLAG, 0, &exact_search_ParISnew_inmemory_geopat);
        }
        else if (function_type == 9999 || function_type == 99999){      // ekosmas's lock-free fai only after help version with lock-free parallelism in subtree and cow
            DO_NOT_HELP = 0;
            ...
            // PHASE 3: Query Answering
            isax_query_binary_file_traditional_ekosmas_lf(queries, queries_size, idx, minimum_distance, LOCKFREE_PARALLELISM_IN_SUBTREE_COW, 0, &exact_search_ParISnew_inmemory_hybrid_ekosmas_lf);
        }
