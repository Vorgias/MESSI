...
main(int argc, char **argv)
{
    ...
    fflush(stdout);
    start = get_microsec();
    if ((kdTree = kd_buildTree(pointlist, npoints, data_constr, data_destr,
                               min, max, dim, nthreads)) == NULL) {
    ...
    if ((result = kd_ortRangeSearch(kdTree, rect_min, rect_max, dim)) == NULL) {
     ...
     */
    range = SQR(2);             /* This has to be bigger than the extent of the
                                 * field. The content of this variable is replaced
                                 * with the distance to the NN squared. */
    printf
    ...
    nearest = kd_nearest(kdTree, rect_min, &range, dim);
     ...
     */
    range = SQR(0.2);           /* range contains the search radius squared */
    ...
    if ((result =
         kd_range(kdTree, rect_max, &range, dim, KD_ORDERED)) == NULL) {
    ...
    if ((result = kd_qnearest(kdTree, rect_max, &range, j, dim)) == NULL) {
